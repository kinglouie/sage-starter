<header>
  <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl p-4">
    <a href="https://flowbite.com" class="flex items-center space-x-3 rtl:space-x-reverse">
      ...
      <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">allure</span>
    </a>
      ...
      <nav class="hidden md:block">
        <ul class="flex flex-row p-0 border-gray-100 rounded-lg space-x-8 rtl:space-x-reverse mt-0 border-0">
          @foreach ($menu->all() as $item)
            <li @class([
                $item->classes,
                'hover:text-blue-500' => !$item->active,
              ...
              <a href="{{ $item->url }}" data-dropdown-trigger="hover"
                ...
                class="flex items-center justify-between py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0">
                {{ $item->label }}
                @if ($item->children)
                ...
                <div id="dropdown-{{ $item->id }}"
                  ...
                  <ul class="py-2 text-sm text-gray-700">
                    @foreach ($item->children as $child)
                      <li @class([
                          $child->classes,
                          'hover:text-blue-500' => !$child->active,
